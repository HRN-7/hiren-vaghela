...
def relevant_prices(feed,context):
    """Latest per real market; grade mismatch is rejected and unknown grades remain unverified."""
    selected={}
    requested_variety=norm(context['variety']);requested_grade=norm(context['grade'])
    ...
    for row in feed.get('records',[]):
        try:
            ...
            if age<0 or age>MAX_PRICE_AGE_DAYS or not math.isfinite(float(row['price'])) or row['price']<=0:continue
            variety=norm(row.get('variety',''));reported_grade=norm(row.get('grade',''))
            if not unspecified and variety!=requested_variety and variety not in generic:continue

            # Farmer A/B/C grade is mandatory for a confirmed recommendation.
            # If the provider explicitly reports A/B/C and it differs, reject it.
            # Other grade systems (for example FAQ) may still be displayed, but
            # are never treated as verified A/B/C matches for net-realization ranking.
            if reported_grade in ['a','b','c'] and reported_grade!=requested_grade:
                continue

            grade_verified=reported_grade in ['a','b','c'] and reported_grade==requested_grade
            ...
            r={
                **row,
                ...
                'grade_match':(
                    ...
                    if grade_verified
                    else "Provider grade is not the same verified A/B/C system; this market cannot be confirmed for net-realization ranking until grade compatibility is verified."
                ),
                'source':feed['source'],
            ...
            # Prefer verified grade, then matching variety, then newest report.
            order=(int(grade_verified),int(exact),r['date'],norm(r.get('variety','')),norm(r.get('grade','')))
            if not current or order>current[0]:selected[r['id']]=(order,r)
        except (KeyError,ValueError,TypeError):continue
    return [item[1] for item in selected.values()]


def market_trend(context,row):
    """Validated AI trend is advisory timing unless a true grade-specific model exists."""
    try:
        ...
        if result.get('status')!='estimated' or not records:
            return {
                ...
                'message':result.get('message','No validated AI trend is available.')
            }

        first=float(records[0]['price']);last=float(records[-1]['price'])
        change_pct=((last-first)/first*100) if first else 0.0

        if change_pct>0.5:signal='rising'
        elif change_pct<-0.5:signal='falling'
        else:signal='stable'
        ...
        return {
            ...
            'recommendation_use':result.get('recommendation_use','timing_trend_only'),
...
async def discover(context):
    ...
    for row,route in zip(found,routes):
        row['distance_km']=route.get('distance_km')
        row['distance_source']='OSRM / OpenStreetMap'
        row['distance_note']='Estimated driving distance to the mapped market/locality; confirm the unloading point and truck route.'
...
async def service_options(context,feed,storage_days):
    ...
    try:
        ...
        return result
    except Exception:return {**result,'message':'Service quotes could not beconfirmed for this crop. Prices remain visible; incomplete options are not recommended.'}
...
def compute_option(market,transport,storage,charges,quantity,storage_days):
    ...
    if not market.get('grade_verified'):
        missing.append('Verified A/B/C grade compatibility')
